...
function populateSpouseSelect(selectEl, gender, excludeIds){
  const opposite = gender === "male" ? "female" : gender === "female" ? "male" : null;
  const candidates = membersFlat.filter(m =>
    !excludeIds.includes(m.id) && (!opposite || m.gender === opposite)
  );
...
/* ---------------- قائمة الأزواج/الزوجات القابلة للتعديل (نافذة التعديل) ---------------- */
let editingSpousesList = [];

function renderEditSpousesList(){
  const wrap = document.getElementById("editSpousesList");
  if(editingSpousesList.length === 0){
    wrap.innerHTML = `<p class="spouse-empty">لا يوجد زوج/زوجة مسجّل حالياً.</p>`;
    return;
  }
  wrap.innerHTML = editingSpousesList.map((s, i) => `
    <div class="spouse-chip">
      <span>${escapeHtml(s.name || "—")}${!s.id ? " (غير مسجّل بالشجرة)" : ""}</span>
      <button type="button" data-remove-spouse="${i}">×</button>
    </div>
  `).join("");
  wrap.querySelectorAll("[data-remove-spouse]").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      editingSpousesList.splice(Number(btn.dataset.removeSpouse), 1);
      renderEditSpousesList();
    });
  });
}

document.getElementById("editNewSpouseMode").addEventListener("change", ()=>{
  const mode = document.getElementById("editNewSpouseMode").value;
  document.getElementById("editNewSpouseExistingWrap").style.display = mode === "existing" ? "block" : "none";
  document.getElementById("editNewSpouseNewWrap").style.display = mode === "new" ? "block" : "none";
  document.getElementById("editAddSpouseBtnWrap").style.display = mode ? "block" : "none";
  if(mode === "existing"){
    const excludeIds = [document.getElementById("editId").value, ...editingSpousesList.map(s => s.id).filter(Boolean)];
    populateSpouseSelect(document.getElementById("editNewSpousePerson"), document.getElementById("editGender").value, excludeIds);
  }
});
document.getElementById("editAddSpouseBtn").addEventListener("click", ()=>{
  const mode = document.getElementById("editNewSpouseMode").value;
  if(mode === "existing"){
    const id = document.getElementById("editNewSpousePerson").value;
    if(!id) return;
    const spouseMember = membersFlat.find(m => m.id === id);
    editingSpousesList.push({ id, name: spouseMember ? (spouseMember.fullName || spouseMember.firstName) : "" });
  } else if(mode === "new"){
    const name = document.getElementById("editNewSpouseNewName").value.trim();
    if(!name) return;
    editingSpousesList.push({ id: null, name });
  }
  renderEditSpousesList();
  document.getElementById("editNewSpouseMode").value = "";
  document.getElementById("editNewSpouseExistingWrap").style.display = "none";
  document.getElementById("editNewSpouseNewWrap").style.display = "none";
  document.getElementById("editAddSpouseBtnWrap").style.display = "none";
  document.getElementById("editNewSpouseNewName").value = "";
});

/* ---------------- قائمة الأزواج/الزوجات (نافذة الإضافة المباشرة) ---------------- */
let directSpousesList = [];

function renderDirectSpousesList(){
  const wrap = document.getElementById("directSpousesList");
  if(directSpousesList.length === 0){
    wrap.innerHTML = `<p class="spouse-empty">بدون.</p>`;
    return;
  }
  wrap.innerHTML = directSpousesList.map((s, i) => `
    <div class="spouse-chip">
      <span>${escapeHtml(s.name || "—")}${!s.id ? " (غير مسجّل بالشجرة)" : ""}</span>
      <button type="button" data-remove-direct-spouse="${i}">×</button>
    </div>
  `).join("");
  wrap.querySelectorAll("[data-remove-direct-spouse]").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      directSpousesList.splice(Number(btn.dataset.removeDirectSpouse), 1);
      renderDirectSpousesList();
    });
  });
}
renderDirectSpousesList();

document.getElementById("directNewSpouseMode").addEventListener("change", ()=>{
  const mode = document.getElementById("directNewSpouseMode").value;
  document.getElementById("directNewSpouseExistingWrap").style.display = mode === "existing" ? "block" : "none";
  document.getElementById("directNewSpouseNewWrap").style.display = mode === "new" ? "block" : "none";
  document.getElementById("directAddSpouseBtnWrap").style.display = mode ? "block" : "none";
  if(mode === "existing"){
    const excludeIds = directSpousesList.map(s => s.id).filter(Boolean);
    populateSpouseSelect(document.getElementById("directNewSpousePerson"), document.getElementById("directGender").value, excludeIds);
  }
});
document.getElementById("directAddSpouseBtn").addEventListener("click", ()=>{
  const mode = document.getElementById("directNewSpouseMode").value;
  if(mode === "existing"){
    const id = document.getElementById("directNewSpousePerson").value;
    if(!id) return;
    const spouseMember = membersFlat.find(m => m.id === id);
    directSpousesList.push({ id, name: spouseMember ? (spouseMember.fullName || spouseMember.firstName) : "" });
  } else if(mode === "new"){
    const name = document.getElementById("directNewSpouseNewName").value.trim();
    if(!name) return;
    directSpousesList.push({ id: null, name });
  }
  renderDirectSpousesList();
  document.getElementById("directNewSpouseMode").value = "";
  document.getElementById("directNewSpouseExistingWrap").style.display = "none";
  document.getElementById("directNewSpouseNewWrap").style.display = "none";
  document.getElementById("directAddSpouseBtnWrap").style.display = "none";
  document.getElementById("directNewSpouseNewName").value = "";
});
  ...
  panelPending.querySelectorAll("[data-approve-spouselink]").forEach(btn=>{
    btn.addEventListener("click", async ()=>{
      ...
      try{
        ...
        const targetSpouses = getSpousesArray(target);
        ...
        if(item.spouseMemberId){
          ...
          if(!targetSpouses.some(s => s.id === item.spouseMemberId)){
            targetSpouses.push({ id: item.spouseMemberId, name: spouse.fullName || spouse.firstName });
            await updateDoc(doc(db, "members", item.targetMemberId), { spouses: targetSpouses, spouseId: null, spouseName: null });
          }
          const spouseSpouses = getSpousesArray(spouse);
          if(!spouseSpouses.some(s => s.id === item.targetMemberId)){
            spouseSpouses.push({ id: item.targetMemberId, name: target.fullName || target.firstName });
            await updateDoc(doc(db, "members", item.spouseMemberId), { spouses: spouseSpouses, spouseId: null, spouseName: null });
          }
          await deleteDoc(doc(db, "pendingSubmissions", id));
          await logActivity("spouse_link",
            `وافق على إضافة "${spouse.fullName || spouse.firstName}" كزوج/زوجة لـ "${target.fullName || target.firstName}"`
          );
        } else {
          if(!targetSpouses.some(s => !s.id && s.name === item.spouseFreeText)){
            targetSpouses.push({ id: null, name: item.spouseFreeText });
            await updateDoc(doc(db, "members", item.targetMemberId), { spouses: targetSpouses, spouseId: null, spouseName: null });
          }
          await deleteDoc(doc(db, "pendingSubmissions", id));
          await logActivity("spouse_link",
...
document.getElementById("dragSpouseBtn").addEventListener("click", async ()=>{
  ...
  const target = membersFlat.find(m => m.id === dragTargetId);
  if(!source || !target) return;
  const sourceSpouses = getSpousesArray(source);
  const targetSpouses = getSpousesArray(target);
  if(!sourceSpouses.some(s => s.id === dragTargetId)){
    sourceSpouses.push({ id: dragTargetId, name: target.fullName || target.firstName });
    await updateDoc(doc(db, "members", dragSourceId), { spouses: sourceSpouses, spouseId: null, spouseName: null });
  }
  if(!targetSpouses.some(s => s.id === dragSourceId)){
    targetSpouses.push({ id: dragSourceId, name: source.fullName || source.firstName });
    await updateDoc(doc(db, "members", dragTargetId), { spouses: targetSpouses, spouseId: null, spouseName: null });
  }
  document.getElementById("dragRelationOverlay").classList.add("hidden");
...
function openEdit(m){
  ...
  populateSpouseSelect(document.getElementById("editNewSpousePerson"), m.gender, [m.id]);
  editingSpousesList = getSpousesArray(m).map(s => ({ ...s }));
  renderEditSpousesList();
  document.getElementById("editNewSpouseMode").value = "";
  document.getElementById("editNewSpouseExistingWrap").style.display = "none";
  document.getElementById("editNewSpouseNewWrap").style.display = "none";
...
document.getElementById("editForm").addEventListener("submit", async (e)=>{
  ...
  const newSpouses = editingSpousesList.map(s => ({ id: s.id || null, name: s.name || "" }));
  ...
  const newVals = {
    ...
    spouses: newSpouses,
    spouseId: null,   // نوقف استخدام الحقول القديمة المفردة لصالح القائمة الجديدة
    spouseName: null,
  ...
  try{
    const oldSpouses = editingSnapshot ? getSpousesArray(editingSnapshot) : [];
    const oldIds = oldSpouses.map(s => s.id).filter(Boolean);
    const newIds = newSpouses.map(s => s.id).filter(Boolean);
    ...
    // الشخص القديم: نشيل ربطه بهذا الشخص من قائمته
    for(const removedId of removedIds){
      const removedMember = membersFlat.find(m => m.id === removedId);
      if(!removedMember) continue;
      const theirSpouses = getSpousesArray(removedMember).filter(s => s.id !== id);
      await updateDoc(doc(db, "members", removedId), { spouses: theirSpouses, spouseId: null, spouseName: null }).catch(()=>{});
    }
    // الشخص الجديد: نضيف ربطه بهذا الشخص لقائمته (لو ما كان موجوداً أصلاً)
    for(const addedId of addedIds){
      const addedMember = membersFlat.find(m => m.id === addedId);
      if(!addedMember) continue;
      const theirSpouses = getSpousesArray(addedMember);
      if(!theirSpouses.some(s => s.id === id)){
        theirSpouses.push({ id, name: thisFullName });
        await updateDoc(doc(db, "members", addedId), { spouses: theirSpouses, spouseId: null, spouseName: null }).catch(()=>{});
      }
    }
    ...
    if(editingSnapshot){
      const fieldLabels = {
        ...
        status: "الحالة", birthYear: "سنة الميلاد", deathYear: "سنة الوفاة",
      ...
      const oldSpouseNames = oldSpouses.map(s => s.name).filter(Boolean).sort().join("، ");
      const newSpouseNames = newSpouses.map(s => s.name).filter(Boolean).sort().join("، ");
      if(oldSpouseNames !== newSpouseNames){
        changes.push(`الأزواج/الزوجات: ${oldSpouseNames || "—"} ← ${newSpouseNames || "—"}`);
      }
...
document.getElementById("directAddForm").addEventListener("submit", async (e)=>{
  ...
  try{
    ...
    const spousesToSave = directSpousesList.map(s => ({ id: s.id || null, name: s.name || "" }));
    ...
    for(const s of spousesToSave){
      if(!s.id) continue;
      const spouseMember = membersFlat.find(m => m.id === s.id);
      if(!spouseMember) continue;
      const theirSpouses = getSpousesArray(spouseMember);
      theirSpouses.push({ id: newRef.id, name: directFullName || directFirstName });
      await updateDoc(doc(db, "members", s.id), { spouses: theirSpouses, spouseId: null, spouseName: null }).catch(()=>{});
    }
